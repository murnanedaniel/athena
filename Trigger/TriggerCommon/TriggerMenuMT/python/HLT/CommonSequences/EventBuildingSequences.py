...
def addEventBuildingSequence(flags, chain, eventBuildType, chainDict):
    ...
    if len(chain.steps)==0:
        ...
        #FPP step_name = 'Step{:d}_PEBInfoWriter_{:s}'.format(len(chain.steps)+1, eventBuildType)
        ...
        #FPP step_name = 'Step{:d}_merged{:s}_PEBInfoWriter_{:s}'.format(len(chain.steps)+1,prevStep.name, eventBuildType)
        step = ChainStep(name=step_name,
