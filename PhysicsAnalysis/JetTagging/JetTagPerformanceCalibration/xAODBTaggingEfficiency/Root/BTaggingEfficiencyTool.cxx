...
StatusCode BTaggingEfficiencyTool::initialize() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (m_systStrategy == "SFEigen" || m_systStrategy == "SFGlobalEigen") {
      ...
      //std::cout << " BTaggingEfficiencyTool constructor -> before m_CDI listScaleFactorUncertainties for flavour " << getLabel(flavourID) << std::endl;
    ...
    //std::cout << " End of flavour loop for flavour " << getLabel(flavourID) << std::endl;
  } // <---- end flavour loop...

  //std::cout << " BTaggingEfficiencyTool constructor -> After flavour loop " << std::endl;
					...
					float & sf) const
{
  ...
  if( m_applySyst) { // indicate that we want to apply a systematic variation, i.e. return an up/down variation pair
    ...
    //std::cout << "BTagEffTool : m_applySyst is " << m_applySyst << " for unc " << unc << ". " << std::endl;
    if(!m_applyThisSyst.getIndex(flavour,unc_ind)) { // But if the flavour is
  ...
  status = m_isContinuous ? m_CDI->getWeightScaleFactor(v,sfindex,efindex, unc,unc_ind,result) : m_CDI->getScaleFactor(v,sfindex,efindex, unc,unc_ind,result, getLabel(flavour)); //<----- getScaleFactor method#3

  // Interpret what has been retrieved;
  // this depends both on the uncertainty type and on the up/down setting.
  sf = result.first; // central value or up variation, checking which it is next...
  if (m_applySyst && unc != None) {
    if (! (unc == SFEigen || unc == SFNamed || unc == SFGlobalEigen)){ 
      sf += m_applyThisSyst.isUp ? result.second : -result.second ; // so if it's a (central,unc) pair, add the uncertainty to the central (up or down)
...
BTaggingEfficiencyTool::getEfficiency( const xAOD::Jet & jet, float & eff) const
{
  ...
  //std::cout << "BTagEffTool->getEfficiency : Jet has pt " << jet.pt() << " eta " << jet.eta() << " phi " << jet.phi() << " m " << jet.m() << std::endl;
  // might have to do this manually here, until I can figure out why the jet version of "fillVariables" throws errors...
  const double pt = jet.pt();
  const double eta = jet.eta();
  const double tagwe = 0.7;
  //if (! fillVariables(jet, vars)) {
  if (! fillVariables(pt, eta, tagwe, vars)){
...
BTaggingEfficiencyTool::fillVariables( const xAOD::Jet & jet, CalibrationDataVariables& x) const
{
  ...
  //bool weightOK = true;
  if (m_isContinuous) {
    //std::cout << " BTagEffTool->fillVariables : m_Continuous is true." << std::endl;
    const xAOD::BTagging* tagInfo = xAOD::BTaggingUtilities::getBTagging( jet ); // <------- Problem with BTagEffToolTester is that this returns null pointer, which happens because the "AuxElement" jet isn't valid?? 
    //std::cout << " BTagEffTool->fillVariables : after getBTagging (jet)" << std::endl;
