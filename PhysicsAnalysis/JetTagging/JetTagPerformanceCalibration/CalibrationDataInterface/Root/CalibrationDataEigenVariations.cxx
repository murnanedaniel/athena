...
namespace {
  ...
  void validate_reduction(int matrixsize, TMatrixDSym& corr, std::vector<std::pair<TH1*, TH1*>>& eigenvars, TH1* result, std::string scheme, std::string tagger, std::string wp, std::string jetauthor){
    ...
    for(int row = 0 ; row < approx_corr.GetNrows() ; row++){
      for(int col = 0 ; col < approx_corr.GetNcols() ; col++){
        //std::cout << " row: " << row << " col: " << col << " -- " << approx_corr(row,col) << ", " << corr(row,col) << " : " << numerator(row,col) << ",  " << denominator(row,col) << std::endl;
    ...
    //std::cout << "The correlation matrices have shapes [" << corr.GetNrows() << ", " << corr.GetNcols() << "] for corr, and [" << approx_corr.GetNrows() << ", " << approx_corr.GetNcols() << "] for approx_corr." << std::endl;
    string outputname = "Validate_" + scheme + "_" + tagger + "_" + wp + "_" + jetauthor + "_" + std::to_string(eigenvars.size()) + "vars" ; 
    string eospath = "/eos/user/w/wleinone/plotting/"; // <------ Place you want to store plots. Obviously, change this (please)
    ...
    fout.cd();
    TH2* hout = new TH2D(correlation_comparison);
    
    // Note: you can also look at the other matrices involved - save them to file and plot them if you wish
    //TH2* hout = new TH2D(cov); 
    //TH2* hout = new TH2D(approx_corr);
    //TH2* hout = new TH2D(corr);
  ...
  TMatrixDSym getSystCovarianceMatrix(const vector<double>& unc){
    ...
    //std::cout << "getSystCovarianceMatrix -> cov has " << cov.GetNrows() << std::endl;
    return cov;
  }
    ...
    m_cnt(cnt), m_initialized(false), m_validate(false), m_namedExtrapolation(-1), m_statVariations(false), m_cdipath(cdipath), m_taggername(tagger), m_wp(wp), m_jetauthor(jetcollection), m_totalvariance(0), m_capturedvariance(0)
{
  ...
  if (excludeRecommendedUncertaintySet && base) {
    ...
    for (auto name : to_exclude) {
      //std::cout << " CalibrationDataEigenVariations ---------------> about to excludeNamedUncertainty [" << name << "]" << std::endl;
  ...
  //std::cout << " CDEV constructor - m_cnt name is " << m_cnt->GetName() << "\n" << std::endl;
...
CalibrationDataEigenVariations::excludeNamedUncertainty(const std::string& name, CalibrationDataContainer* cnt)
{
  ...
  if (m_initialized) std::cerr << "CalibrationDataEigenVariations::excludeNamedUncertainty error:" << " initialization already done" << std::endl;
  
  else if (name == "comment"    || name == "result"   || name == "systematics" || name == "statistics" || name == "combined" || name == "extrapolation" || name == "MCreference" || name == "MChadronisation" || name == "ReducedSets" || name == "excluded_set" || name == "" || name == " ")
...
CalibrationDataEigenVariations::getEigenCovarianceMatrix() const
{
  ...
  for (unsigned int t = 0; t < uncs.size(); ++t) {
    ...
    if (uncs[t] == "comment" || uncs[t] == "result" || uncs[t] == "combined" || uncs[t] == "statistics" || uncs[t] == "systematics" || uncs[t] == "MCreference" || uncs[t] == "MChadronisation" || uncs[t] == "extrapolation" || uncs[t] == "ReducedSets" || uncs[t] == "excluded_set") continue;
...
CalibrationDataEigenVariations::getJacobianReductionMatrix() const
{
  ...
  for (int i = 0; i < nbins; ++i) {
    ...
    if ((binx == 0 || binx == result->GetNbinsX()+1) || (ndim > 1 && (biny == 0 || biny == result->GetNbinsY()+1)) || (ndim > 2 && (binz == 0 || binz == result->GetNbinsZ()+1))) {
      ++nZeros;
  ...
  int size = nbins - nZeros; // <-------------- Ok, so this is the size of the matrix removing zeros from the covariance matrix... Next?
  ...
  }                                                //             <-------- This matrix only add 1's on/off diagonal, upper triangular matrix. OR is it symmetric???
...
CalibrationDataEigenVariations::initialize(double min_variance)
{
  ...
  //std::cout << " calculating variation size from result bins -> " << result->GetNbinsX()*result->GetNbinsY()*result->GetNbinsZ() << " compared to " << cov.GetNrows() << ", " << cov.GetNcols() << std::endl;

  //std::cout << " Check out axis labels: X " << result->GetXaxis()->GetTitle() << ", Y " << result->GetYaxis()->GetTitle() << ", Z " << result->GetZaxis()->GetTitle() << std::endl;
  //TH2* covTH2 = new TH2D(cov);
  //std::cout << "covTH2 has " << covTH2->GetNbinsX() << ", " << covTH2->GetNbinsY() << ", " << covTH2->GetNbinsZ() << std::endl;
  ...
  //std::cout << "count of non-zero elements of corr is " << coun << std::endl;
  //*/
  ...
  for (int i = 0; i < matrixVariationsWithZeros.GetNrows(); ++i) {
    ...
    for (int u = 0; u < nbins; ++u) { // <------ Should this be starting from 0? Yes.
  ...
  for (size_t index = 0; index < m_eigen.size(); ++index) {
    ...
    //for (int bin = 1 ; bin <= nbins ; ++bin){ // <------ print out the bin contents if you're curious
    //  std::cout << " eigenvar " << index << " -> bin " << bin << " == " << up->GetBinContent(bin) << std::endl;
    //}
    ...
    if (!keep_variation){ // <------ At this stage, if we find no bins in the systematic with large enough variation, we insert it to "final_set" for removal/pruning
      ...
      //std::cout << " captured variance now " << m_capturedvariance << " from " << eigenValues[index] << std::endl;
    }
    ++current_set;
  }
  if (final_set.size() > 0) 
...
CalibrationDataEigenVariations::mergeVariations(const IndexSuperSet &set)
{
  ...
  for (IndexSuperSet::iterator set_it = set.begin(); set_it != set.end(); ++set_it) {
    ...
    size_t lowest_index = *set_it->lower_bound(0); // get the lowest index variation?
    TH1 *total_var_up = static_cast<TH1*>(m_eigen[lowest_index].first->Clone()), // clone it
							 ...
							 std::map<std::string, std::map<std::string, float>> &coefficientMap) const
{
  ...
  for (unsigned int t = 0; t < fullUncList.size(); ++t) {
    // entries that should never be included
    if (fullUncList[t] == "comment" || fullUncList[t] == "result" || fullUncList[t] == "combined" || fullUncList[t] == "statistics" || fullUncList[t] == "systematics" || fullUncList[t] == "MCreference" || fullUncList[t] == "MChadronisation" || fullUncList[t] == "extrapolation" || fullUncList[t] == "ReducedSets" || fullUncList[t] == "excluded_set") continue;
    // entries that can be excluded if desired
...
//________________________________________________________________________________
CalibrationDataGlobalEigenVariations::CalibrationDataGlobalEigenVariations(std::string cdipath, std::string tagger, std::string wp, std::string jetcollection, const CalibrationDataHistogramContainer* cnt, bool excludeRecommendedUncertaintySet) : 
...
//m_blockmatrixsize(1200)
{
  ...
  for (auto const& flavour : m_flavours){
    ...
    //std::cout << "Retrieving container for " << dir <<  std::endl;
    ...
    if (c) {
      ...
      for (std::string unc : uncs){
        ...
        if ((unc=="result")||(unc=="comment")||(unc=="ReducedSets")||(unc=="systematics")||(unc=="statistics")||(unc=="extrapolation")||(unc=="MChadronisation")||(unc=="combined")||(unc=="extrapolation from charm")) {  // <---- Do I need to check for extrapolation from charm?
          continue; 
        } else { 
      ...
      if (excludeRecommendedUncertaintySet) {
        ...
        for (auto name : to_exclude) {
          if (name == "") continue;
          //std::cout << " CDGEV about to excludeNamedUncertainty [" << name << "] for flavour " << flavour << std::endl;
          excludeNamedUncertainty(name, flavour);//, const_cast<CalibrationDataHistogramContainer*>(c)); // modified to pass the container directly to the method
        }
      }
    }
  }
...
CalibrationDataGlobalEigenVariations::~CalibrationDataGlobalEigenVariations()
{
  ...
  for (auto flavour : m_flavours){
    std::cout << "flavour eigenvariations destructor: removing " << m_flav_eigen[flavour].size() << " " << flavour << " eigenvariations" << std::endl;
...
CalibrationDataGlobalEigenVariations::getEigenCovarianceMatrix()
{
  ...
  for (std::string unc : m_all_shared_systematics){
      ...
      if (c) {
        ...
        if (hunc){
          ...
          if (ref){
            ...
            //return global_covariance; // <------ This should be essentially empty... But it also could be partly filled? Need to fix with exception handling!
          }
        }
        // Now we can loop through the bins of the flavour uncertainty, adding them onto the combined systematic
        //std::cout << "Found " << flavour_size << " bins in flavour " << flavour << " and unc " << unc << std::endl;
        //std::cout << "CDGEV->getEigenCovarianceMatrix : tagweight is " << tagweightax << std::endl;
          ...
          for (Int_t xbins = 1 ; xbins <= ref->GetNbinsX() ; xbins++){
            for (Int_t zbins = 1 ; zbins <= ref->GetNbinsZ() ; zbins++){
              for (Int_t ybins = 1 ; ybins <= ref->GetNbinsY() ; ybins++){
                //std::cout << " unc is " << unc << " xbins " << xbins << " ybins " << ybins << " zbins " << zbins << std::endl;
                if (hunc){
                  Int_t bin = hunc->GetBin(xbins,ybins,zbins); 
                  double unc_val = hunc->GetBinContent(bin); 
                  comb_syst.push_back(unc_val); // if uncertainty, push uncertainty bin content to combined systematic vector 
                  //std::cout <<  unc << " " << unc_val << " : xbins " << xbins << " ybins " << ybins << " zbins " << zbins << std::endl;
                } else {
                  comb_syst.push_back(0.0); // if no uncertainty, push 0's
                  //std::cout << unc << " 0.0 : xbins " << xbins << " ybins " << ybins << " zbins " << zbins << std::endl;
                }
        ...
        //cout << "vector<double> comb_syst has length " << comb_syst.size()  << endl;
      }
    }
    ...
    //std::cout << "comb_syst has " << comb_syst.size() << " elements, now constructing unc_cov..." << std::endl;
    if (unc == "statistics"){
    ...
    //std::cout << "After getSystCovarianceMatrix -> unc_cov has " << unc_cov.GetNrows() << " rows" << std::endl;
    ...
    if (flavs_in_common[0]+flavs_in_common[1]+flavs_in_common[2]+flavs_in_common[3] > 1){
      //cout << "- - - -> Adding " << unc << " to shared_cov" << endl;
      m_only_shared_systematics.insert(unc) ; // mark the shared systematics...
    }
    //std::cout << "global_covariance has " << global_covariance.GetNrows() << " rows, while unc_cov has " << unc_cov.GetNrows() << " rows" << std::endl;
    global_covariance += unc_cov;
    
    // <------- Now let's see which flavours this uncertainty applies to! (just commenting out below to reduce output clutter)
    //std::cout << std::setw(40) << unc << " is in flavours [ " << flavs_in_common[0] << ", " << flavs_in_common[1] << ", " << flavs_in_common[2] << ", " << flavs_in_common[3] << " ]" << std::endl;
  }

  /*
  // Now, if you want to save the individually constructed combined covariance matrices to file so you can view them later, do the following
  std::string eospath = "/eos/user/w/wleinone/plotting/" + m_taggername + "/" + m_jetauthor + "/" + m_wp + "/";
  std::cout << "Entering covariance matrix print out section... " << std::endl;
  for(std::string unc : m_all_shared_systematics){
    string foutputname = unc;
    std::cout << "ATTEMPTING TO WRITE " << unc << " TO FILE..." << std::endl;
    //std::string flavs = ((flavs_in_common[0]==1) ? "B" : "");// Write BLT if uncertainty goes to B, L, and T
    //flavs = flavs + ((flavs_in_common[1]==1) ? "C" : "");
    //flavs = flavs + ((flavs_in_common[2]==1) ? "L" : "");
    //flavs = flavs + ((flavs_in_common[3]==1) ? "T" : "");
    //string outputfilename = eospath + foutputname + "_flavs" + flavs + ".root";
    string outputfilename = eospath + foutputname + ".root";
    //string uncfilename = eospath  + outputname + "_flavs" + flavs + "_unc" + unc + ".root";
    TFile fout(outputfilename.c_str(), "recreate");
    fout.cd();
    TH2* hout = new TH2D(cov_matrices[unc]);
    hout->SetTitle((unc).c_str());
    hout->Write();
    fout.Close();
  }
  //*/
...
CalibrationDataGlobalEigenVariations::excludeNamedUncertainty(const std::string& name, const std::string& flavour)//, CalibrationDataContainer* cnt)
{
  ...
  //std::cout << "------------------------> excludeNamedUncertainty adding " << name << " to flavour " << flavour << std::endl;
  ...
  //else if (! cnt->GetValue(name.c_str()))
  //  std::cerr << "CalibrationDataGlobalEigenVariations::excludeNamedUncertainty error: uncertainty named " << name << " not found in flavour " << flavour << std::endl;
  // only really add if the entry is not yet in the list
    ...
    // Can also add to "m_namedIndices" with dummy index of 0, in order for "listNamedVariations" and other methods to work
    //if (m_namedIndices.find(name)==m_namedIndices.end()){ // only add this once...
    //  m_namedIndices[name] = 0; 
    //}
  }
}
...
CalibrationDataGlobalEigenVariations::initialize(double min_variance)
{
  ...
  // Second step: create the variations for the named sources of uncertainty (easy...)
  // <---- News flash: This isn't so easy now, as it has to be donefor all the flavours to which the uncertainty pertains
  ...
  for (auto const& flavour : m_flavours) {
    ...
    if (c->getTagWeightAxis() == -1){ // <---- For fixed cut WP, the Y axis **should** be the pT axis (but can it can potentially be different?)
    ...
    for (map<string, unsigned int>::iterator it = m_flav_namedIndices[flavour].begin(); it != m_flav_namedIndices[flavour].end(); ++it) {
      ...
      if (hunc){
        //std::cout << " CDGEV->initialize : varying hunc " << namedvar << " for flavour " << flavour << std::endl;
        resultVariedUp->Add(hunc, 1.0);    resultVariedUp->SetDirectory(0);
        resultVariedDown->Add(hunc, -1.0); resultVariedDown->SetDirectory(0);
      } else {
        //std::cout << " CDGEV->initialize : hunc " << it->first << " didn't exist for flavour " << flavour << ", adding a zero variation."<< std::endl;
  ...
  //std::cout << " matrixCovariance has " << matrixCovariance.GetNrows() << " rows and " << matrixCovariance.GetNrows() << " cols" << std::endl;
  // Carry out the Eigenvector decomposition on this matrix
  TMatrixDSymEigen eigenValueMaker (matrixCovariance);
  TVectorT<double> eigenValues   = eigenValueMaker.GetEigenValues(); 
  TMatrixT<double> eigenVectors  = eigenValueMaker.GetEigenVectors(); 
  ...
  for (int i = 0; i < size; ++i) {
    //now go back
    //std::cout << " eigenValue " << i << " has value " << eigenValues[i] << " and fabs value " << fabs(eigenValues[i]) << std::endl;
    for (int r = 0; r < size; ++r) {
      //first index is the variation number, second corresponds to the pT bin      //<----------- The "eigenvariations" matrix is the "C" matrix which has CKC^T = I with "K" being the o.g. covariance matrix, and "I" is the identity.
      matrixVariations(i,r) = -1.0*eigenVectors[r][i]*sqrt(fabs(eigenValues[i])); // <----------------- So the result is a matrix (eigenvariation) which is the eigenvector scaled by the sqrt(eigenvalue)
    } 
  ...
  //std::cout << "m_eigen has " << m_eigen.size() << " variations before any pruning " << std::endl;
  ...
  for (size_t index = 0; index < m_eigen.size(); ++index) {
    ...
    up->Add(comb_result, -1.0); // now we're left with decimal values centered around 0, i.e. 0.02 or -0.02

    for (int bin = 1; bin <= nbins; ++bin) {
      if (fabs(up->GetBinContent(bin)) > min_variance) { // <--------- If you find even ONE bin with big enough variance, we keep the whole systematic.
        //std::cout << "CDGEV->initialize : eigenvar " << index << " bin " << bin << " value is " << up->GetBinContent(bin) << std::endl;
        keep_variation = true;
        break;
      }
    }
    if (!keep_variation){ // <--------------- At this stage, if we find no bins in the systematic with large enough variation, we insert it to "final_set" for removal/pruning
      ...
      //std::cout << " captured variation is now " << m_capturedvariance << " from " << std::setprecision(9) << eigenValues[index] << std::endl;
    }
    delete up;
    ++current_set;
  }
  ...
  for(std::pair<TH1*,TH1*> var : m_eigen){
    ...
    for (std::string flavour : m_flavours){
      ...
      //std::cout << "Pushing the " << flavour << " " << eigenvarup << " to m_flav_eigen : bin_baseline is now " << bin_baseline << std::endl;
      ...
      //std::cout << flavour << " result ----> Xbins" << result->GetNbinsX() << " Ybins " << result->GetNbinsY() << " Zbins " << result->GetNbinsZ() << std::endl; // just to check...
      //std::cout << flavour << " result ----> dimension " << result->GetDimension() << std::endl; // just to check...
      //std::cout << flavour << " " << eigenvarup << ": eigenvarup ----> Xbins" << resultVariedUp->GetNbinsX() << " Ybins " << resultVariedUp->GetNbinsY() << " Zbins " << resultVariedUp->GetNbinsZ() << std::endl; // just to check...
      //std::cout << flavour << " " << eigenvarup << ": eigenvarup ----> dimension " << resultVariedUp->GetDimension() << std::endl; // just to check...
    }
  }


  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ////// The flavour-specific variations are now ordered in descending significance, as per the global eigenvalue decomposition.      //////
  ////// But the contents of these flavour-specific variations may not actually reflect that, since the variance contribution         //////
  ////// may come from one flavour exclusively, hence leaving the other flavours with next-to-nothing in terms of variation from      //////
  ////// zero.                Should these not be further sorted into the descending significance as per a "local" EV decomp.?        //////
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  /*
  for (auto flav : m_flavours){
    // for each flavour, we want to prune remaining variations and keep only the non-zero ones?
    std::vector<std::pair<TH1*, TH1*>> eigenvars = m_flav_eigen[flav];
    std::cout << "----------- " << flav << " -----------" << std::endl;
    int varn = 0;
    for (auto var : eigenvars){
      std::cout << " eigenvariation index is " << varn << std::endl; varn++;
      TH1* up = var.first;
      TH1* down = var.second;
      std::cout << " up has " << up->GetNbinsX() << ", " << up->GetNbinsY() << ", " << up->GetNbinsZ() << std::endl;
      for(int binx = 1 ; binx <= up->GetNbinsX(); binx++){
       for(int biny = 1 ; biny <= up->GetNbinsY(); biny++){
        for(int binz = 1 ; binz <= up->GetNbinsZ(); binz++){
            Int_t bin = up->GetBin(binx,biny,binz);
            double ub = up->GetBinContent(bin); 
            double db = down->GetBinContent(bin);
            std::cout << " bin # " << bin << " : " << ub << " , " << db << std::endl;
          }
        } 
      }
    }
...
//________________________________________________________________________________
TMatrixD
CalibrationDataGlobalEigenVariations::getJacobianReductionMatrix(TMatrixDSym& cov) const
{
  // Construct the matrix that removes the rows and columns that fail to influence
  // the eigen-variations. To reduce the covariance matrix, do the following:
...
//________________________________________________________________________________
unsigned int
CalibrationDataGlobalEigenVariations::getNumberOfEigenVariations(const std::string& flavour) const
{
  if (! m_initialized) const_cast<CalibrationDataGlobalEigenVariations*>(this)->initialize();
  return (m_flav_eigen.find(flavour)->second).size();
}

//________________________________________________________________________________
bool
CalibrationDataGlobalEigenVariations::getEigenvectorVariation(const std::string& flavour, unsigned int variation, TH1*& up, TH1*& down) const
{
  // Return the pointers to the up- and downward variation histogram for the specified
  // eigenvector variation. In case of an invalid variation number, null pointers will
  // be returned and the return value will be false.
  ...
  //     up:          (reference to) pointer to upward variation histogram
  //     down:        (reference to) pointer to downward variation histogram
  //std::cout << " CDGEV -> getEigenvectorVariation #1" << std::endl;
  if (! m_initialized) const_cast<CalibrationDataGlobalEigenVariations*>(this)->initialize();
  //std::cout << " CDGEV -> getEigenvectorVariation #2" << std::endl;
  std::vector<std::pair<TH1*,TH1*>> flav_variations = m_flav_eigen.find(flavour)->second;
  if (variation < flav_variations.size()){ //[flavour].size()) {
    up   = flav_variations[variation].first;
    down = flav_variations[variation].second;
    //std::cout << " CDGEV -> getEigenvectorVariation returned... what's their value?" << std::endl;
    return true;
  }
...
//________________________________________________________________________________
bool
CalibrationDataGlobalEigenVariations::getNamedVariation(const string& flavour, const string& name, TH1*& up, TH1*& down)// const
{
  // Return the pointers to the up- and downward variation histogram for the specified
  // named variation. In case of an invalid named variation, null pointers will
  // be returned and the return value will be false.
...
//________________________________________________________________________________
bool
CalibrationDataGlobalEigenVariations::getNamedVariation(unsigned int nameIndex, const std::string& flavour, TH1*& up, TH1*& down)// const // <-------------- Need to figure out the const correct way
{
  // Return the pointers to the up- and downward variation histogram for the specified
  // named variation. In case of an invalid named variation number, null pointers will
  // be returned and the return value will be false.
...
CalibrationDataGlobalEigenVariations::getNamedVariationIndex(const std::string& name, const std::string& flavour) const
{
  // Return the integer index corresponding to the named variation.
  // Note that no checks are made on the validity of the name provided.
  //std::cout << " --------------> CDGEV->getNamedVariationIndex : flavour " << flavour << " has " << (m_flav_namedIndices.find(flavour)->second).size() << " named vars :  called with name [" << name << "]" << std::endl;
  std::map<std::string, unsigned int>::const_iterator it = (m_flav_namedIndices.find(flavour)->second).find(name);
  //std::cout << " --------------> CDGEV->getNamedVariationIndex : value of " << it->second <<  std::endl;
  return it->second;
}
...
// I've temporarily made these virtual methods - may have to overload "mergeVariationsFrom" to specify the flavour?
//________________________________________________________________________________
void
CalibrationDataGlobalEigenVariations::mergeVariationsFrom(const size_t& index, std::string& flav)
{
  ...
  //std::cout << " CDGEV -> mergeVariationsFrom : calling to merge variations from the " << index << " bin up for flavour " << flav << "!!!" << std::endl;
  IndexSet simple_set;
...
CalibrationDataGlobalEigenVariations::mergeVariations(const IndexSuperSet &set, std::string& flav)
{
  ...
  //for (std::string flavour : m_flavours){
    ...
    for (IndexSuperSet::iterator set_it = set.begin(); set_it != set.end(); ++set_it) {
      ...
      total_var_up->Reset(); // I suppose the following for-loop
      total_var_down->Reset();
      ...
      for (IndexSet::iterator subset_it = set_it->begin(); subset_it != set_it->end(); ++subset_it) {
        ...
        partial_var_up->Add(result, -1.0); // <---- This doesn't seem correct, no?
        partial_var_down->Add(result, -1.0);
        for (int i = 0; i < nbins; ++i) {
          partial_var_down->SetBinContent(i, -1.0*partial_var_down->GetBinContent(i));
        }
...
{
  if (set.size() == 0) return;
  //std::cout << " flavour " << flav << " initially has " << m_flav_eigen[flav].size() << " variations" << std::endl;
  std::vector<std::pair<TH1*, TH1*> > new_eigen;
  ...
  //std::cout << "-------> flavour " << flav << " now has " << m_flav_eigen[flav].size() << " variations" << std::endl;
